...
export async function search(data, original = {}) {
  ...
  for (const result of results) {
    ...
    for (const match of matches) {
      ...
      let possibleName = possibleMatch.name;
      if (possibleName.includes("(Remaster)")) { // Ignore remaster on the title since sometimes there's no track without it
        possibleName = possibleName.replace(" (Remaster)", "")
      }
      const nameMatch = possibleName == data.name;
